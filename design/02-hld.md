...
| delivery summary | `C: stationary (declared)`, `A: multicast along py (derived)`, `B: multicast along px (derived)` |
...
| delivery summary | `U: stationary (declared)` — the delivery block is one mechanical `<operand>: <how> (<declared>)` line per operand and nothing else; the halo is visible in the `ToNorth`/`ToSouth` channel rows above. *(Erratum, 2026-09-13, the ruling on **B-P22**: `exchange("U", …)` names `U`'s delivery, so under R2 — "`declared` is a fact about the schedule: a clause names this operand's delivery" — the row is `("U", STATIONARY, None, True)`. The earlier two-facts-in-one-sentence form predates the one-bit `declared`.)* |
...
| channel `EastOut` | `size=[1]`, L1→L3 — the drain from PE 3, `MQ` gets at segment scope, into `S[i, NR:NR+1]`. *(Erratum, 2026-09-13, ruling **R-W3-1**: the drain writes the tail PE's own east edge column of `S` — the same value its `SOut` put also writes — so the plan needs no synthetic `Sink` tensor and `tensors` stays `(q, r, S)`. Measured: `aircc --output-format=none` exit 0 with zero `error:` lines on npu1 and npu2, and `air-verify-hierarchy-locality{strict=true}`, which `aircc` runs on the placed IR by default, prints nothing.)* |
...
| delivery summary | `S: forward along px (declared)`, `q: multicast along px (derived)`, `r: stationary (derived)` — one mechanical line per operand, in `MappingPlan.delivery` order; the wavefront's shape is in the `WestIn`/`West`/`EastOut` channel rows above. *(Erratum, 2026-09-13, the ruling on **B-P22**: `forward("S", …)` both names and replaces `S`'s row under R2 — "`declared` is a fact about the schedule: a clause names this operand's delivery".)* |
