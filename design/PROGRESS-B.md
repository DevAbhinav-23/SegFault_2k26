...
| **B-P29** *(new)* | **CLOSED 2026-09-15** — `tests/unit/test_nfr5_constants.py` is the AST lint. An UPPER_CASE module-level assignment in `spatial/*.py` must be followed by a string literal; a **run** of consecutive assignments shares the docstring that ends it, because that is how the source already groups `_READ_ALIGN`/`_WRITE_ALIGN`. Measured: **90** constants, **16** undocumented, all 16 in modules that pass did not own — 9 A's (`m1` 1, `m2` 5, `m3` 3), 6 M0's (frozen), 1 C's (`m6_tools._DEFAULT_FIX`) — listed one per line in `KNOWN_UNDOCUMENTED` with their owner. Two of B's own were fixed rather than listed (`m5tt_emit._HEADER`, `m6tt_run._ENV_HINT`). The list is a **ratchet**: a new offender fails the first test, and documenting a listed one fails the second until its line is deleted. **The final pass paid the debt off**: all 16 carry their docstring and `KNOWN_UNDOCUMENTED` is **empty**, the lint asserting it stays empty. The six in `model.py` are private constants — a docstring on one changes no field of any record, so the frozen contract is untouched. `test_NFR5_the_exemption_list_only_shrinks` became one test over the set rather than a `parametrize`, because a `parametrize` over an empty set is a *skip* and would read in the tally as a gap | §P7 → 2026-09-15. **B** |
...
| **B-P32** *(new)* | **CLOSED 2026-09-15 — the emitter widens a load to its destination's dtype.** `_expr`'s `Load` arm wraps the value in `air.api.ops.cast(value, <destination dtype>)` **only when** the two differ, so "bf16 in, f32 out" reaches `air.api`, which has no mixed-precision elementwise assignment (`_emit.py:458-471`: *"dtype mismatch in elementwise assignment: destination is air.api.f32 but operand is air.api.bf16"*). Ruling: cast each **load**, not the product — the accumulate is then f32 throughout, which is what the phrase means. The condition is an inequality, so **every golden is byte-identical** (`git diff main -- tests/golden/*` is empty) and `ops.cast` joins `test_m5_emit.ALLOWED_AIR_NAMES`. `kernels/w1_gemm_bf16.schedule_os` is real: it plans and emits on npu1 and npu2, **two `arith.extf`, no `truncf`, 200 lines**. **`aircc` still refuses it, and the measurement says it is the shape, not the dtype**: npu1 `air-to-aie` *"'air.channel.get' op failed to get MM2S tile for L3 allocation"*, npu2 `aiecc` *"'aie.tile' op allocated buffers exceeded available memory"* (65 536 B of a 64 KB tile) — and a cast-free **f32 control** at the same 4×4 grid and the same 49 152 B of L1 (`TK = 32`) draws the *same two diagnostics*, which is what `tests/integration/test_kernels_live.py::test_live_bf16_gemm_is_refused_by_aircc_for_its_shape` asserts. `w1_large` therefore stays levels I and S, as `03-lld-M8-kernels-demo.md` §4 has it | §P7 → 2026-09-15. **B** |
| **B-P33** *(new)* | **MEASURED 2026-09-15 — the npu1 refusal of W1-large is not a DMA-channel resource, so ruling R-P3-2 was *not* applied.** `air-to-aie` fails at `AIRToAIESchedulingUtils.cpp:3892-3894` — `if (f.MM2S_alloc.empty() || !f.MM2S_alloc[0].getDmaTile()) return memcpyOpIf->emitOpError("failed to get MM2S tile for L3 allocation.");` — on the launch-side `air.channel.get @C2L3[0,0]`. The producer alloc is empty because the **herd-side put's bundle index is still a loop induction variable**: with `repeats == (4,1)` the placed IR carries `%29 = affine.apply ()[%arg10, %arg14] -> (s0*4 + s1)` inside a surviving `scf.for %arg14`, so `specializeChannelBundle` cannot resolve bundle position `[0,0]` to a producer. Upstream names this exact cause in its own regression: `mlir/test/Conversion/AIRToAIE/segment_id_remap_no_unroll.mlir:14-19`, *"channel operations using them as indices fail to specialize, causing: 'air.channel.get' op failed to get MM2S tile for L3 allocation."* **Discriminator measured, five shapes, same tiles (`TM=TN=TK=64`, bf16 in / f32 out) and the same 49 152 B of L1:** `repeats[0] == 1` (grid 1×2) → `aircc` **exit 0** on both targets; `repeats[0] == 2` (grid 2×2, 2×4) → past `air-to-aie`, dies at buffer assignment; `repeats[0] == 4` (grid 4×1, 4×2, 4×4) → this error. The `air` ping-pong unroll factor is 2, which *fully* unrolls a trip-2 repeat loop (the two puts then carry `s0*2` and `s0*2+1`, constant per core) and leaves a trip-2 loop when the repeat is 4. **Why R-P3-2 does not fit:** the count of L3 flows is not what separates the passing from the failing shapes — grid 2×4 (8 outbound C2L3 flows) passes `air-to-aie` and grid 4×2 (also 8) does not — so extending P3's `DMA-CHANNELS` would name a resource that is not the one exhausted, and §3.8's own argument ("a checker that rejects a program the toolchain accepts is worse than no checker") cuts the other way here too. The honest check is a **bundle-index** condition on `repeats`, which is `06-interfaces.md` §5.6 invariant 3's territory, not §3.8's; it is not written because the unroll factor 2 is measured on this wheel, not a documented contract (R-19/R-21). `tests/integration/test_kernels_live.py::test_live_bf16_gemm_is_refused_by_aircc_for_its_shape` keeps the failing pair and gains `test_live_bf16_gemm_compiles_when_the_grid_fits_the_herd` as the passing control. **CLOSED 2026-09-15 by ruling R-HERD-1** (`../hackathon/HANDOFF.md`, Rulings): the bundle-index condition *is* written, as a condition on `repeats` rather than on the index — a repeat factor above the unroll factor 2 is rejected in `m3_legality._check_repeats` with `HERD-PHYSICAL`. Controls, both measured this pass and both `slow`+`requires_aircc`: `test_repeat_four_is_rejected_and_repeat_two_is_not` (checker) and `test_repeat_four_is_what_aircc_refuses` (`aircc` on npu1: `repeats (4,1)` → *"'air.channel.get' op failed to get MM2S tile for L3 allocation"*, `repeats (2,1)` → exit 0, same tiles and clauses). The rejected module is built with `_check_repeats` monkeypatched off, so the evidence stays a measurement | §P8 → 2026-09-15 → **closed 2026-09-15**. **B** |
| **B-P34** *(new)* | **MEASURED 2026-09-15 — the npu2 refusal is `sum(buffers) + 2048 stack > 65536`, and ruling R-L1-2 was applied; a second, larger gap is recorded and *not* closed.** mlir-aie's `AIEAssignBuffers.cpp` prints its own arithmetic: `(stack) : 0x0-0x7FF (2048 bytes)` then six buffers ending at `0x107FF` = 67 583, against `AIE2TargetModel::getLocalMemorySize()` = `0x10000`. The 2 048 B is `air-to-aie`'s `stack-size` option default (mlir-air `Passes.td:231-234`, *"Default is 2048 bytes"*), written as `stack_size = 2048 : i32` onto every `aie.core` (`AIRToAIEPass.cpp:415-416`) — a pass option, not a device fact, so it is the same on both generations. **Applied:** contract **v7**, the binding budget is 63 488 in `m3_legality._check_l1_capacity` (`L1-CAPACITY`, whose message now states the reserved amount) and in `m4_mapping.L1_USABLE` / `m4_selfcheck.L1_USABLE`. `L1_BUDGET` keeps naming the tile's 65 536 B because `MappingSummary.l1_budget` and the `"L1: n of 65536 bytes"` summary line are frozen in `tests/golden/*.plan.json` and `*.summary.txt`; nothing in the suite sits between 63 488 and 65 536 (largest is W1-large at 49 152, then W1-flip 24 576). **What this does not fix, and is the real gap:** our `l1_bytes` is **per logical PE**, and a physical core that serves `repeats` logical PEs allocates `repeats` accumulators. Measured on the lowered IR: at `repeats == (1,1)` a tile carries 5 buffers = 49 152 B (= `l1_bytes`, `aircc` exit 0); at `repeats == (2,1)` it carries **6** — the accumulator twice, the ping-pong operands once — = 65 536 B, and the plan is unlinkable while `l1_bytes` still says 49 152. Charging `repeats` would change `l1_bytes` for W1-base on npu1 (12 288 → 16 384) and with it every plan and summary golden, so it is **not** done here: it is an M3/M4 accounting change that needs its own ruling and a golden regeneration. **CLOSED 2026-09-15 by ruling R-L1-3** (`../hackathon/HANDOFF.md`, Rulings): every L1 buffer is charged **twice** when any repeat factor exceeds 1 — not the accumulator alone, and never four times. Re-measured on the lowered IR this pass, `aie.buffer` per tile: W1 base npu2 `repeats (1,1)` → **5 buffers, 12 288 B** (acc once, `a`/`b` ping-ponged); W1 base npu1 `repeats (2,1)` → **6 buffers, 16 384 B** = 2 × (4096 + 2048 + 2048); W1-large npu2 `repeats (2,1)` → **6 buffers, 65 536 B** = 2 × (16 384 + 8 192 + 8 192); W1-large-at-`grid(1,2)` `repeats (1,1)` → **5 buffers, 49 152 B**. The mechanism is the ping-pong machinery: it labels one `scf.for` per nest with `unroll = 2` (*"int unroll_factor = 2; // Unroll factor hardened as 2"*, `AIRDependencyScheduleOpt.cpp:1906-1908`; the pass is `Passes.td:916-923`), and at `repeats > 1` that loop is the repeat loop `air.api`'s `run_strip_mined` emits (`_trace.py:1675`), so the *placed* IR carries two copies of the whole herd body — measured, `placed.w1base.npu1.mlir` has two acc allocs and two in-loop `a`/`b` allocs and **no** hoisted ping-pong pair, where `placed.w1base.npu2.mlir` has one acc and four hoisted halves. Charged in `m3_legality._check_l1_capacity` (`L1-CAPACITY`, message naming the unroll and its source) and in `m4_mapping.l1_total` / `m4_selfcheck.l1_total` / `MappingSummary`, which agree by construction. Two goldens moved, on their L1 numbers only | §P8 → 2026-09-15 → **closed 2026-09-15**. **B** |
| **B-P35** *(new)* | **CLOSED 2026-09-15 — two defects in M4 on degenerate grids, both fixed with regression tests.** (1) **A single-PE grid tripped M4's own self-check.** W1's clauses at `grid(1, 1)` raised `PROTOCOL-UNSUPPORTED: site 'C2L3.get.0@segment' carries order 0 but sits at index 3 of its body`, on both targets. Cause: `_sites` gave every segment-scope site `order = 0`, which is right only when the site sits **inside** a wrapper — `_bundle_nest` builds no loop for a bundle dim of extent 1, so with every extent 1 the drain `get` lands in the segment body itself and its order is its index there (`06-interfaces.md` §5.2). Fixed by counting the segment body the way `PROTOCOL` builds it. (2) **A 1-D `grid(1,)` raised a wrapped `NotImplementedError`** — *"the streamed buffers ['a', 'b'] are re-fetched per different axes"*. Cause: `temporal_axes` counted an outer tile axis of **extent 1** (`tile(ax.j, 64)` at `N = 64`) as temporal, so `B[k,j]` "moved" with `j0` while `A[i,k]` moved with `k0`, and the fill/compute/drain shape has one streaming loop. A one-tile axis makes a one-trip loop and moves nothing — it is the same schedule as not tiling `j` — so it is dropped from `T`. The genuinely unsupported case (two staged operands moving with two axes that both have trips) is now a user-facing `PROTOCOL-UNSUPPORTED` naming both axes and the clause to edit, not a `NotImplementedError` wrapped in a bug-report `fix` (NFR-7). **Tests** (`tests/integration/test_kernels_live.py`): `grid(1,1)`, `grid(1,)` and `grid(2,)` each plan, carry a site order equal to its index in its body, emit on both targets, run through `tests/helpers/plan_interp` to the **exact** CPython `A @ B` (`np.array_equal`), and compile — `aircc --output-format=none` **exit 0, six of six** (three shapes × two targets) | §P8 → 2026-09-15. **B** |
| **B-P36** *(new)* | **CLOSED 2026-09-15 — `aiecc` refuses a four-row single-column herd, and the count of packet flows decides it *once gated on a multicast*; the bare count does not, which is what the first pass measured and could not explain.** `grid(1, 4)` (physical `(1, 4)`, `repeats (1, 1)`, `l1_bytes` 12 288) fails on **both** targets inside `AIEPathfinderPass::runOnPacketFlow`: `'aie.tile' op tile op arbiter 0 has used up all its msels`, pointing at `%tile_0_2 = aie.tile(0, 2)` — the bottom tile of the column every L3→L1 flow climbs through. The resource **is** quotable: mlir-aie's `lib/Dialect/AIE/Transforms/AIECreatePathFindFlows.cpp` declares `int numMselsPerArbiter = 4;` and emits that message when a new packet flow needs an amsel on a full arbiter; the `aie.amsel` op's `msel` is `ConfinedAttr<AIEI8Attr, [IntMinValue<0>, IntMaxValue<3>]>` (`AIEOps.td:615`, pinned wheel). A GEMM wall was measured exactly there — `aie.packet_flow` counted in `air_project/aie.*.mlir`, W1's clauses, both targets: `(1,2)` **3 flows → exit 0**; `(1,3)` **4 → exit 0**; `(1,4)` **5 → msels**; also `grid(2,4)` on npu1 (physical `(1,4)`) **5 → msels**, and W1 base's own `(1,2)` physical herd **3 → exit 0**. **Why no rule:** the flip's 2-D variant (`w1flip_legal.legal(target, grid2d=True)`, `grid(1, 4)`, the shape `test_W1_flip_2d_aircc_none` already compiles) puts **8** packet flows into that same one column and `aircc` **exits 0 on both targets**. So the demand is not the flow count: an amsel is shared by flows whose switchbox output-port set matches (*"This packet flow switchbox's output ports completely mismatches with any existing amsel. Creating a new amsel"*), which is a property of the routes the pathfinder chooses, not of the plan. A `DMA-CHANNELS` rule counting flows per column was written, measured against the flip, and **reverted**: it would have rejected a program the toolchain accepts, which §3.8 says is worse than no checker **The rule, and what the earlier pass missed: the demand *is* plan-level; what the earlier pass missed is that a multicast stops the sharing.** Re-measured on the **routed** IR rather than on the flow list: `aie-opt --aie-create-pathfinder-flows=route-packet` over `air_project/npu.src.mlir`, counting `aie.amsel<arbiter>(msel)` on `%tile_0_2`. W1's clauses, physical `(1, 2)`: 3 fill flows, amsels `<0>(0)`, `<0>(1)`, `<0>(2)` — **3 on arbiter 0**, exit 0. Physical `(1, 3)`: 4 flows, `<0>(0..3)` — **4, exactly the cap**, exit 0, reached both as `grid(1, 3)` and as `grid(2, 3)` folded with `repeats (2, 1)` (which emits the same four `aie.packet_flow`). Physical `(1, 4)`: 5 flows → *used up all its msels*, on **both** generations, whether reached as `grid(1, 4)` or `grid(2, 4)`. Physical `(2, 4)` from `grid(4, 4)` on npu2: 5 flows → the same error. Physical `(2, 2)` on npu2: 4 flows → exit 0. **The flip's exemption, explained:** its eight flows are every one of them single-destination (`aie.packet_flow` with one `packet_dest`), so their switchbox port sets match exactly and they share **2** amsels — `<0>(0)` for `{DMA}` and `<1>(0)` for `{North}`, on two different arbiters. The GEMM's `A2L1` is a *multicast*: its port set at the bottom tile is `{DMA, North}`, which only **partially** overlaps every point-to-point flow, and a partial overlap opens a fresh amsel on the same arbiter instead of sharing one. So the demand is the fill-flow count **gated on a column multicast**, and the capacity is `numMselsPerArbiter = 4`. **Applied:** `m4_selfcheck.msels` + `fill_flows` + `fans_down_the_column`, raising `DMA-CHANNELS`; `fill_flows` counts one flow per bundle position whose repeat factor is 1 (a repeat leaves an unresolvable index and the bundle stays one flow) and reproduces the emitted `aie.packet_flow` count on all six shapes above. Controls in `tests/integration/test_kernels_live.py`: `test_four_row_column_is_refused_for_its_master_selects` (5 vs 4) and `test_three_row_column_sits_on_the_master_select_cap` (4, accepted), plus the `slow` `test_the_master_select_cap_is_where_aircc_stops` which prints both `aircc` verdicts on both targets, and the pre-existing `test_W1_flip_2d_aircc_none` as the eight-flow leg. **Not modelled:** that every fill flow enters the same shim column — true of all six shapes (every `aie.packet_source` is `%shim_noc_tile_0_0`) but the shim bin packing's choice, `AIRToAIESchedulingUtils.cpp:3825-3845`, not the plan's | §P8 → 2026-09-15 → **closed 2026-09-15**. **B** |
| **B-P37** *(new)* | **CLOSED 2026-09-15 — `air-to-aie` refuses a herd with more rows than columns, at the *fill* side, and the resource is an upstream affine bound rather than a shim bin.** On npu2, `grid(2, 3)` (physical `(2, 3)`) and `grid(2, 4)` (physical `(2, 4)`), both `repeats (1, 1)` and `l1_bytes` 12 288, fail with `'air.channel.put' op failed to get S2MM tile for L3 allocation` — the S2MM twin of B-P33's message, `AIRToAIESchedulingUtils.cpp:3866-3868`, on the **second bundle position of the multicast `A2L1` put**. The surrounding comment states the resource: *"L3 shim allocation is bin-packing onto a fixed set of ShimNOC cols (hard cap = device.getNumShimNOCCols(), per-bin cap = 2 MM2S + 2 S2MM)"*, in two passes, *"pass 1 — rigid (non-broadcast L3 MM2S + all L3 S2MM), pass 2 — flexible (broadcast L3 MM2S), reusing existing bins via the broadcast cross-bucket fallback"*. **Why no rule:** the capacity is quotable but our demand is not countable against it. npu2 is 8 columns of ShimNOC tiles (`NPU2TargetModel::columns() == 8`, every row-0 tile a `ShimNOCTile`), which is 16 S2MM against the 6–8 this plan needs, so the module is not over the *global* cap; what fails is a per-bin, per-column assignment inside the allocator, on a flow whose own S2MM side is an L1 get. Measured discriminator, same clauses and tiles: physical `(2, 2)` → **exit 0** (4 packet flows, 4 drains); `(2, 3)` → this error; `(2, 4)` → this error; `(1, 4)` → past `air-to-aie` (it dies later, B-P36). A rule would have to model the allocator's bin choice, which no upstream document states **The rule: it is not a shim resource at all, it is an upstream bound with the wrong dimension.** The allocator was traced as the brief asked and cleared: `AIRToAIESchedulingUtils.cpp:3863-3868` is inside the `f.MM2S_memspace == L3` branch, where a put from L3 asks for the **destination's** S2MM tile to know which column to route to; it fails because `f.S2MM_alloc[i]` was never filled, i.e. broadcast position `i` of that flow has **no receiver**, not because a bin ran out. The dumped placed IR says so directly — segment `{x_loc = 0, x_size = 8, y_loc = 2, y_size = 6}` on npu2, so eight shim columns for a plan needing at most six S2MM, and the herd at `{x_loc = 0, y_loc = 2}` with cores `(0..1, 2..4)`. **The cause** is `air-specialize-dma-broadcast`: splitting a bundled multicast wraps each split get in an `affine.if` whose set pins the specialised herd coordinate and bounds every **other** one by `numCols - 1` (`mlir/lib/Transform/AIRMiscPasses.cpp:265-271`), the `numCols` argument being `herd.getNumCols()` (`:347-349`) whichever dimension is bounded. At physical `(2, 3)` the emitted set is `#set = affine_set<()[s0, s1] : (s0 == 0, s1 >= 0, -s1 + 1 >= 0)>` — rows 0..1 of a 3-row herd — and the placed IR shows the consequence: `aie.core(%tile_0_4)` reads `@A2L1_1`, the **other** column's A block, while `@A2L1_0` is left with 2 of its 3 broadcast destinations. **Pinned by construction:** editing only that one bound to `-s1 + 2 >= 0` and re-running `air-to-aie` on the same placed IR gives **exit 0**. Measured table, W1's clauses, npu2: physical `(2, 2)` exit 0; `(2, 3)` and `(2, 4)` the S2MM error; `(3, 2)` and `(4, 2)` — six and eight cores, transposed, with `_PHYSICAL_HERD['npu2'][2]` monkeypatched to `(4, 2)` — **exit 0**, which is the measured route to the full device and is *not* adopted here (see the open item below). npu1 is untouched because its 2-D physical herd is one column, so every multi-column logical grid arrives with a repeat on the split axis, the bundle index is an `affine.apply` rather than a bare herd id, and upstream takes its `scf.if` arm (`AIRMiscPasses.cpp:380-422`) which bounds nothing — measured, `grid(2, 2)` and `grid(2, 3)` compile on npu1 at physical `(1, 2)` and `(1, 3)`. **Applied:** `m4_selfcheck.broadcast_guard` + `specialize_dim`, raising `DMA-CHANNELS` with the channel, its `size`/`broadcast_shape`, the split dimension, the rows admitted and the rows unserved. Controls in `tests/integration/test_kernels_live.py`: `test_tall_herd_is_refused_for_the_broadcast_guard_on_npu2`, `test_a_repeat_on_the_split_axis_takes_the_guard_out_of_range`, `test_square_herd_is_taken_by_the_checker`, and the `slow` `test_tall_herd_is_refused_by_aircc_and_the_square_one_is_not` which prints the `aircc` verdicts for `grid(2, 4)` and `grid(2, 2)` on npu2. **Options (a) and (b) of the brief were both checked and are not available:** (a) the segment is *not* short of shim columns, so there is nothing for the emitter to ask for — and a logical `grid(2, 4)` cannot be given more than two columns anyway, because `air.api` requires the physical shape to divide the logical grid per axis (`_trace.py:1345-1372`); (b) `aircc --omit-auto-broadcast`, the one documented switch that skips this pass, was measured and still fails the same way, because `air-to-aie`'s own `specializeChannelBundle` then cannot resolve the bundle either | §P8 → 2026-09-15 → **closed 2026-09-15**. **B** |
| **B-P38** *(new)* | **open — the measured route to npu2's full 8-PE 2-D herd, needing an architect ruling.** B-P37 makes `grid(2, 4)` — `air.api`'s own npu2 2-D default, 2 columns by 4 rows — unusable, and the logical grid cannot be widened because the physical shape must divide it per axis. What *does* work is the transpose: with `_PHYSICAL_HERD['npu2'][2]` / `_PHYSICAL_CAP[('npu2', 2)]` monkeypatched from `(2, 4)` to `(4, 2)`, `grid(4, 2)` resolves to physical `(4, 2)` — the same **eight** cores — and `aircc --device npu2 --output-format=none` **exits 0**; `grid(3, 2)` at physical `(3, 2)` likewise. Not adopted, for two reasons. (1) A blanket cap swap regresses other shapes: `grid(1, 4)` would fold to `(1, 2)`, halving the flip's 2-D herd, and `grid(2, 3)` would fold to `(2, 1)` with `repeats (1, 3)` and be refused by R-HERD-1. Expressing "eight cores, any aspect with columns >= rows" needs a shape-aware resolver and a tie-break policy, not a per-axis tuple. (2) The table is **`air.api`'s own** (`_trace.py:88-91`), quoted in `03-lld-M3-checker.md` §3.11, in R-HERD-1's diagnostic and in FR-L10, so changing it is an architect decision. The alternative — having M4 transpose the herd silently when `grid[1] > grid[0]` — would change the emitted text for every 2-D plan of that shape and reverses what `place(px=, py=)` records (`03-lld-M2-schedule.md` §3.4), so it is also not something this pass may do | §P8 → 2026-09-15. **Architect**, then **B** |
...
| **B-O8** | **CLOSED 2026-09-15** — *answered: `check_pin` + byte-for-byte goldens are the standing answer; a pin change is a golden regeneration by definition.* There is nothing further to measure: `test_T6_versions` runs first and skips the golden tests when the installed version is not the pin, `--update-goldens` refuses outright on a mismatch (`tests/conftest.py`, layer 2), and any `str(module)` drift inside the pin shows as a failing byte-for-byte golden — which is the signal, not a surprise | P1 → 2026-09-15. **Answered** |
